...
class Decoder
{
    ...
    protected function validateSignature($recievedSignature, $resultedSignature)
    {
        if ($resultedSignature !== $recievedSignature) {
            throw new JwtException('Token is invalid. Received signature is not equal to resulted signature.', 401);
        }
    }
    ...
    {
        if (empty($token)) {
            throw new JwtException('Token is empty.', 404);
        }
    }
    ...
    protected function validateTokenParts(array $tokenParts)
    {
        if (count($tokenParts) !== 3) {
            throw new JwtException('Token is invalid. It is not composed of three parts.', 401);
        }
    }
